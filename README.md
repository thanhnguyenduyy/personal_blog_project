...
4. **Cấu hình Mail (SMTP):**
   - Cập nhật thông tin cấu hình Email trong file `.env` để sử dụng tính năng gửi mail:
   ```env
   MAIL_MAILER=smtp
   ...
   **Cách lấy MAIL_PASSWORD (Google App Password):**
   - Truy cập vào [Tài khoản Google](https://myaccount.google.com/).
   - Chọn mục **Bảo mật** (Security).
   - Đảm bảo bạn đã bật **Xác minh 2 bước** (2-Step Verification).
   - Tìm kiếm "Mật khẩu ứng dụng" (App Passwords).
   - Tạo mật khẩu mới với tên ứng dụng tùy chọn (ví dụ: "Laravel Blog").
   - Sao chép mã 16 ký tự được cung cấp (ví dụ: `abcd dtds gggg uayo`) và dán vào `MAIL_PASSWORD` trong file `.env`.
